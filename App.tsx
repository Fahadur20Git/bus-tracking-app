...
import { BusRoute, AppState, LiveBus, BusType } from './types';
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          setState(prev => ({ ...prev, isLoading: false, error: "Failed to connect to AI Service" }));
        }
      },
      (err) => {
    ...
    <div className="flex flex-col h-screen max-h-screen bg-white font-sans text-sm">
      ...
      <nav className="bg-emerald-800 text-white/70 flex text-[10px] font-black uppercase tracking-tighter shrink-0 shadow-lg relative z-40 overflow-x-auto no-scrollbar">
        ...
        <button 
          ...
          className={`flex-1 py-4 px-6 border-b-4 transition-all whitespace-nowrap ${state.viewMode === 'search' ? 'border-white text-white bg-emerald-700' : 'border-transparent hover:bg-emerald-700/50'}`}
        >
          🔍 {t.tabs.search}
        </button>
        <button 
          onClick={() => setState(prev => ({ ...prev, viewMode: 'board' }))}
          className={`flex-1 py-4 px-6 border-b-4 transition-all whitespace-nowrap ${state.viewMode === 'board' ? 'border-white text-white bg-emerald-700' : 'border-transparent hover:bg-emerald-700/50'}`}
        >
          📅 {t.tabs.board}
        </button>
        <button 
